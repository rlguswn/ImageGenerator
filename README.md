...
> PyTorch (CUDA 12.4) 설치에는 시간이 걸릴 수 있습니다.
...
서버가 시작되면 `http://127.0.0.1:8000` 에서 FastAPI 문서를 확인할 수 있습니다.
...
![txt2img](docs/screenshots/txt2img.jpg)

#### 화면 구성

- **왼쪽 패널**: 프롬프트 입력 및 설정값 조정
  - 기본 / 고급 / LoRA 탭으로 구분
- **중앙**: 생성된 이미지 표시 (생성 시간 · seed 표시)
- **오른쪽 사이드바**: 생성 현황 (진행률, 썸네일, seed, 소요 시간)

#### 프롬프트 작성 요령

- **영어 태그**를 쉼표로 구분해서 입력합니다.
- SD 1.5 계열 모델은 **77 토큰 제한**이 있어 중요한 태그를 앞에 배치하세요.
...
- **부정 프롬프트**: 제외하고 싶은 요소를 나열합니다.
  ...
  ```
- 가중치 조정: `(keyword:1.3)` 형식으로 특정 태그의 영향력을 높이거나 낮출 수 있습니다.
...
| **Steps** | 디노이징 반복 횟수. 높을수록 품질↑, 속도↓ | `20` ~ `40` |
| **CFG Scale** | 프롬프트 충실도. 높을수록 프롬프트를 강하게 반영 | `6` ~ `7.5` |
| **Sampler** | 이미지 생성 알고리즘 | `DPM++ 2M Karras` |
| **Seed** | 재현성 제어. `-1`이면 매번 다른 이미지 | `-1` (랜덤) |
| **Batch Size** | 한 번에 생성할 이미지 수 | `1` ~ `4` |
| **Clip Skip** | CLIP 레이어 스킵 수. 애니/아시아 특화 모델은 `2`가 적합 | `1` ~ `2` |
...
![img2img](docs/screenshots/img2img.jpg)
...
4. **생성** 버튼을 누릅니다.
5. 결과 이미지 상단의 **결과 / 비교** 버튼으로 원본과 비교할 수 있습니다.
...
3. 가능하면 배경이 단순하고 피사체가 명확한 이미지를 사용하세요.
...
앱 최초 실행 시 나타나는 화면입니다. 10초 카운트다운 후 자동으로 시작되며, **지금 시작** 버튼으로 바로 실행할 수 있습니다.

| 항목 | 설명 |
|---|---|
| **포트 번호** | 백엔드 서버 포트 (기본값: `8000`). 사용 가능 여부 실시간 확인 |
| **베이스 모델** | `models/base/` 폴더의 모델 선택 |
| **정밀도** | `fp16` (권장) / `fp32` |
| **VRAM 최적화** | VRAM 부족 시 활성화. Attention Slicing + VAE Slicing 적용 |
| **CPU 오프로드** | VRAM이 매우 부족할 때 일부 연산을 CPU로 위임 (속도 저하) |

#### 시작 화면

![loading](docs/screenshots/loading.jpg)

앱 시작 시 3단계로 진행 상황을 표시합니다:

1. **Python 엔진 시작** — 백엔드 프로세스 실행
2. **서버 응답 대기** — FastAPI 서버 준비 확인
3. **SD 모델 로딩** — 선택한 모델 GPU 로딩 (캐시 있으면 수초, 최초 실행 시 수십 초 소요)

#### 모델 교체 (런타임)

설정 탭에서 앱을 재시작하지 않고 다른 모델로 전환할 수 있습니다.
...
| AI 엔진 | PyTorch 2.6 (CUDA 12.4), diffusers, transformers, PEFT |
